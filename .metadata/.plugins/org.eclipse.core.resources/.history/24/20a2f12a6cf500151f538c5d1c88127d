...
struct barrier_t barrier_ballBrick;  // synchronise all bricks and ballthreads.
struct barrier_t barrier_bricks;  // synchronise all bricks threads.
struct barrier_t barrier_score;  // synchronise all bricks threads score calculation.
struct barrier_t barrier_colour;  // signal that the colours resources for the bricks is ready
...
int msgQueue_receiveBricks(int msgQ_brick_id, int colThreads, bricks_msg *allBricks_msg)
{
  int iterator;

  for(iterator = 0 ; iterator < colThreads ; iterator++)
  {
    if( msgrcv( msgQ_brick_id, &(allBricks_msg->brick_msg[iterator]), sizeof(brick_msg), 0,0 ) != sizeof(brick_msg)) 
    {
      return XST_FAILURE;
    }
  }
