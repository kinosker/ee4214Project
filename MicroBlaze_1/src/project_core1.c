...
barrier_t barrier_all_threads_end;  // ensure all threads complete execution each iteration
barrier_t barrier_all_threads_start;
...
{   // This thread is statically created (as configured in the kernel configuration) and has priority 0 (This is the highest possible)
	...
	if( myBarrier_init(&barrier_all_threads_end, MAX_BRICKS_THREAD + MAX_CONTROLLER_THREAD + MAX_BALL_THREAD) != 0) // barrier to synchronise all bricks thread..
	{
		xil_printf("Error when initializing barrier...\r\n");
	}


	if( myBarrier_init(&barrier_all_threads_start, MAX_BRICKS_THREAD + MAX_CONTROLLER_THREAD + MAX_BALL_THREAD) != 0) // barrier to synchronise all bricks thread..
	{
		xil_printf("Error when initializing barrier...\r\n");
	}
					...
					while(brickLoop)
					{
						...
						if(brickLoop & 0b1) // if got brick then check
						{
							...
							if(temp_sideHit != 0)
							{
								...
								pthread_mutex_unlock(&mutex_bricks); //insert by kh
...
//			xil_printf("Thread exiting\n");
			...
			myBarrier_decreaseSize(&barrier_all_threads_end);

			myBarrier_decreaseSize(&barrier_all_threads_start);
